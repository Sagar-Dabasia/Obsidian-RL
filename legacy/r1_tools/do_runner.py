...
# I will write a patch for tools/run_trend_backtest.py

with open("tools/runner_impl.py", "w") as f:
    f.write('''
    ...
    if args.manifest:
        ...
        if not isinstance(components, list):
            print("Error: components not a list")
            sys.exit(1)
        ...
        if isinstance(start_ts, bool) or not isinstance(start_ts, int) or \\
           ...
           isinstance(row_count, bool) or not isinstance(row_count, int):
            print("Error: boolean/non-integer timestamps or counts")
            sys.exit(1)
    ...
    if args.manifest and manifest_component:
        if len(bars) != manifest_component.get("row_count"):
            print("Error: stored item count different from row count")
            sys.exit(1)
        if bars[0].timestamp_utc != manifest_component.get("start_timestamp_utc"):
            ...
            sys.exit(1)
        if bars[-1].timestamp_utc != manifest_component.get("end_timestamp_utc"):
            print("Error: runtime first/last timestamps different from manifest")
            sys.exit(1)
    ...
    if args.manifest:
        if not digest_match:
            print("Error: runtime digest mismatch")
            sys.exit(1)
    ...
    # Determine market_model and allow_short
    allow_short = True
