# PPO Seed Stability Pilot 01 Plan and Execution Report

## Overview

This report documents the PPO Seed Stability Pilot 01 evaluation across 5 seeds (42, 7, 23, 101, 202) under the hardened nested walk-forward protocol.

## Experiment Plan

- **Asset / Interval**: BTCUSDT, 15-minute candles
- **Data Start**: 2023-01-01T00:00:00Z (`1672531200000` ms)
- **Reserved Holdout Start**: 2025-07-01T00:00:00Z (`1751328000000` ms)
- **Train Window**: 365 days (`35040` candles)
- **First Purge Gap**: 15 days (`1440` candles, `WARMUP_ROWS`)
- **Inner Evaluation Window**: 60 days (`5760` candles)
- **Second Purge Gap**: 15 days (`1440` candles, `WARMUP_ROWS`)
- **Outer Validation Window**: 90 days (`8640` candles)
- **Step Size**: 180 days
- **Existing Seed**: Seed 42 from `artifacts/walkforward/wf-20260722-160014-022860-7a725ec7.json`
- **Additional Seeds Trained**: 7, 23, 101, 202
- **Timesteps per Fold**: 100,000
- **Environments (`n_envs`)**: 1
- **Device**: auto
- **Expected Folds per Seed**: 3
- **Cost Model**: Default pessimism (5.0 bps fee, 0.5 bps half-spread, 1.0 bps slippage)
- **Scenarios Evaluated**: `base`, `costs2x`, `delay1`
- **Purpose**: Evaluate multi-seed stability across 5 seeds without touching holdout data or tuning parameters after seeing results.
- **Holdout Access**: Prohibited

---

## Status

Training Pending
