...
MARKET_FEATURE_FORMULAS: dict[str, str] = {
    ...
    "breakout_96": "(close[t] - rolling_min(low, 96)) / (rolling_max(high, 96) - rolling_min(low, 96)); 0.5 when range=0",
}
...
PORTFOLIO_BOUNDS: dict[str, dict[str, Any]] = {
    "exposure": {
...
CANDLE_VALIDATION_RULES: dict[str, str] = {
...
def _build_descriptor() -> dict[str, Any]:
    ...
    return {
        ...
        "required_candle_columns": REQUIRED_CANDLE_COLUMNS,
        "market_features": MARKET_FEATURES,
        "market_feature_formulas": MARKET_FEATURE_FORMULAS,
        "log_return_lags": LOG_RETURN_LAGS,
        "rolling_windows": ROLLING_WINDOWS,
        "warmup_rows": WARMUP_ROWS,
        "clip": CLIP,
        "clipped_features": CLIPPED_FEATURES,
        "unclipped_features": UNCLIPPED_FEATURES,
        "portfolio_features": PORTFOLIO_FEATURES,
        "portfolio_bounds": PORTFOLIO_BOUNDS,
        "observation_order": ALL_FEATURES,
        "observation_dim": OBSERVATION_DIM,
        "observation_dtype": OBSERVATION_DTYPE,
        "candle_validation_rules": CANDLE_VALIDATION_RULES,
    }
...
def schema_fingerprint() -> dict[str, Any]:
    ...
    out = dict(desc)
    out["schema_sha256"] = schema_sha256(desc)
    return out
...
def validate_fingerprint(stored: object) -> None:
    ...
    if stored_ver != SCHEMA_VERSION:
        raise RuntimeError(
            f"legacy schema version: expected {SCHEMA_VERSION!r}, got {stored_ver!r} — retrain required"
        )
    ...
    if stored.get("portfolio_bounds") != PORTFOLIO_BOUNDS:
