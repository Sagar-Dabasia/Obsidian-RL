"""Binance USD-M Futures public market-data provider adapter.

Reads completed OHLCV candles from `https://fapi.binance.com/fapi/v1/klines`.
Reads funding rates from `https://fapi.binance.com/fapi/v1/fundingRate`.
"""
...
class BinanceFuturesProvider(BaseRestProvider, MarketDataProvider):
    """Canonical read-only provider for Binance USD-M Futures public data."""

    provider_name: str = "BINANCE_FUTURES"
    ...
    def fetch_bars(
        ...
        """Fetch completed Binance USD-M Futures klines for symbol within [start_ms, end_ms)."""
        ...
        if tf_enum not in self._SUPPORTED_TIMEFRAMES:
            raise UnsupportedSymbolTimeframeError(
                f"Timeframe {tf_enum.value} is not supported by BinanceFuturesProvider"
            )
        ...
        while cursor_ms < end_ms:
            ...
            if not isinstance(payload, list):
                raise MalformedResponseError(
                    f"Expected list from Binance /fapi/v1/klines, got {type(payload).__name__}"
                )
            if not payload:
                break
            ...
            for row in payload:
                if not isinstance(row, (list, tuple)) or len(row) < 12:
                    ...
                    raise MalformedResponseError(
                        f"Malformed Binance row: expected >= 12 items, got {actual_len}"
                    )
    ...
    def fetch_funding_rates(
        ...
        """Fetch Binance USD-M Futures funding rates for symbol within [start_ms, end_ms)."""
        if not isinstance(symbol, str) or not symbol.strip():
            raise ValueError("symbol must be a non-empty string")
        ...
        collected_rates: list[FundingRate] = []
        cursor_ms = start_ms
        current_time_ms = self._current_time_provider()
        ...
        while cursor_ms < end_ms:
            ...
            if not isinstance(payload, list):
                raise MalformedResponseError(
                    f"Expected list from Binance /fapi/v1/fundingRate, got {type(payload).__name__}"
                )
            if not payload:
                break
            ...
            for row in payload:
                if not isinstance(row, dict):
                    raise MalformedResponseError(
                        f"Malformed Binance funding row: expected dict, got {type(row).__name__}"
                    )

                try:
                    funding_time = int(row["fundingTime"])
                    rate_val = float(row["fundingRate"])
                except (ValueError, TypeError, KeyError) as exc:
                    raise MalformedResponseError(
                        f"Malformed Binance funding row values: {row!r}"
                    ) from exc

                observed_at_utc = funding_time
                if observed_at_utc > current_time_ms:
                    continue

                if not math.isfinite(rate_val):
                    raise MalformedResponseError(f"Non-finite funding rate detected: {rate_val}")

                # Deduplicate check (Binance API can return same boundary in overlapping pagination)
                if collected_rates and funding_time <= collected_rates[-1].timestamp_utc:
                    continue
                ...
                collected_rates.append(rate_obj)
        ...
        return tuple(r for r in collected_rates if start_ms <= r.timestamp_utc < end_ms)
