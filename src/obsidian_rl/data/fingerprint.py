...
Provides canonical JSON serialization and deterministic SHA-256 fingerprinting for
cross-asset data contracts (`MarketBar`, `EventNewsItem`) while ensuring hash calculation
excludes self-hash fields (`row_hash`, `raw_content_hash`).
"""
...
def canonical_json(data: Any) -> bytes:
    ...
    if dataclasses.is_dataclass(data) and not isinstance(data, type):
        ...
        raise TypeError(
            f"Expected dict or dataclass instance, got {type(data).__name__}"
        )
...
def compute_canonical_sha256(
    data: Any,
    exclude_keys: Sequence[str] = ("row_hash", "raw_content_hash"),
) -> str:
    """Compute the deterministic 64-character lowercase SHA-256 hex digest of a data object.

    Excludes the specified keys (such as self-hash fields) before canonical JSON serialization.
    """
    if dataclasses.is_dataclass(data) and not isinstance(data, type):
        payload = dataclasses.asdict(data)
    elif isinstance(data, dict):
...
def compute_event_news_hash(item: "EventNewsItem | dict[str, Any] | Any") -> str:
    """Compute the deterministic SHA-256 hash of an EventNewsItem excluding `raw_content_hash`."""
    return compute_canonical_sha256(item, exclude_keys=("raw_content_hash",))
...
def verify_contract_hash(contract: Any) -> bool:
    ...
    if hasattr(contract, "row_hash"):
        ...
        return True
    elif hasattr(contract, "raw_content_hash"):
        stored = contract.raw_content_hash
        ...
        raise ValueError(
            f"Contract {type(contract).__name__} has neither 'row_hash' nor 'raw_content_hash'"
        )
