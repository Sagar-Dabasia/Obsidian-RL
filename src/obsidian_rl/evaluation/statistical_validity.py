...
def compute_psr(
    sr: float, sr_benchmark: float, num_obs: int, skewness: float, kurtosis: float
) -> float:
    """
    ...
    for val in [sr, sr_benchmark, skewness, kurtosis]:
        if math.isnan(val):
    ...
    if sr_var <= 0:
        raise ValueError("Calculated variance must be strictly positive.")

    # Calculate the test statistic (Z-score)
    t_stat = (sr - sr_benchmark) / math.sqrt(sr_var)

    # Return the CDF of the standard normal distribution
    return float(NormalDist().cdf(t_stat))


def expected_maximum_sr(
    trials_sr_var: float, num_trials: int, euler_gamma: float = 0.5772156649
) -> float:
    """
    ...
    if num_trials == 1:
        return 0.0
    if math.isnan(trials_sr_var) or math.isinf(trials_sr_var):
        raise ValueError("trials_sr_var must be finite.")
    if trials_sr_var <= 0:
        raise ValueError("trials_sr_var must be strictly positive for multiple trials.")

    # Z-scores for approximation
    z_inv_1 = NormalDist().inv_cdf(1.0 - 1.0 / num_trials)
    z_inv_2 = NormalDist().inv_cdf(1.0 - 1.0 / (num_trials * math.e))

    expected_max = math.sqrt(trials_sr_var) * (
        (1.0 - euler_gamma) * z_inv_1 + euler_gamma * z_inv_2
    )
    return float(expected_max)


def compute_dsr(
    sr: float, trials_sr_var: float, num_trials: int, num_obs: int, skewness: float, kurtosis: float
) -> float:
    """
    ...
    Returns:
        DSR (float) representing the probability that the true SR > 0,
