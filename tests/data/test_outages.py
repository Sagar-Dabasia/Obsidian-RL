...
def test_empty_response_hash_cannot_authorize() -> None:
    ...
    for o in outages:
        assert (
            o.source_content_hash
            != "85192d96c8756283f3f316b7144f90967f7b9fd2146d8ae1880eee98a2ff2486"
        )
...
def test_invalid_entries_absent_from_active_registry() -> None:
    ...
    reg = default_registry()
    # The verified BINANCE outages and the new DUKASCOPY holidays should exist
    assert len(reg._outages) == 7
    venues = [o.venue for o in reg._outages]
    assert "BINANCE_SPOT" in venues
    assert "DUKASCOPY" in venues
...
def test_march_2019_archive_fixture_confirms() -> None:
    ...
    assert (
        BINANCE_2019_03_12_OUTAGE.source_content_hash
        == "fed5735ec622a9a18ea5a131e7c334642a44e3a27a8fe2710d8009da8e21c6b9"
    )
...
def test_no_synthetic_row_introduced() -> None:
    ...
    verify_and_digest_continuous_bars(
        bars, eval_start_ms, Timeframe.H4, "BINANCE_SPOT", outage_registry=reg, min_warmup_bars=720
    )
