...
def test_runner_manifest_validation(tmp_path):
    ...
    # Valid run
    res = run_backtest_with_manifest(
        str(db_path),
        str(manifest_path),
        **{
            "start-ms": bars[0].timestamp_utc,
            "end-ms": bars[-1].timestamp_utc + 4 * 3600 * 1000,
            "eval-start-ms": eval_start,
        },
    )
    ...
    res = run_backtest_with_manifest(
        ...
        str(manifest_path),
        **{
            "start-ms": bars[0].timestamp_utc - 4 * 3600 * 1000,
            "end-ms": bars[-1].timestamp_utc + 4 * 3600 * 1000,
            "eval-start-ms": eval_start,
        },
    )
...
def test_manifest_overwrite_protection(tmp_path, monkeypatch) -> None:
    ...
    with open("tools/build_trend_pilot_dataset.py") as f:
        src = f.read()
    ...
    assert (
        ...
        or "already exists" in res.stdout
        or "FileExistsError" in res.stderr
    )
